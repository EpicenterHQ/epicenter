/**
 * The one body codec the platform can ship.
 *
 * A table's body codec says what its node MEANS, and the platform cannot
...
import type { BodyCodec, BodyError } from './declaration.js';

/**
 * A body node that is its sequence, read and written as one string.
 *
 ...
 * holds. Whole rather than diffed, because this codec's body IS one string
 ...
 * the codec for a table whose body is exactly its text, and what makes it
...
export function plainText(): BodyCodec {
	return {
		...
		decode: (text): Result<Y.Node, BodyError> => {
			const node = new Y.Node();
			if (text !== '') node.insert(0, [text]);
			return Ok(node);
		},
		rewrite: (node, text): Result<void, BodyError> => {
