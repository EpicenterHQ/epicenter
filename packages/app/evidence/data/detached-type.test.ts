/**
 * What a detached `Y.Node` can and cannot do, pinned.
 *
...
function integrate(type: Y.Node): Y.Node {
	const document = new Y.Doc({ gc: true });
	const root = document.get('root');
	document.transact(() => {
		root.setAttr('r' as never, type as never);
	});
	return root.getAttr('r' as never) as Y.Node;
}

const named = (...names: string[]) => names.map((name) => new Y.Node(name));

describe('a detached type is SAFE for', () => {
	test('one bulk insert, which is what `pmnodeToDelta` produces', () => {
...
describe('a detached type is UNSAFE for', () => {
	test('repeated positional appends, which REVERSE and do not throw', () => {
		...
		const attached = integrate(new Y.Node());
...
describe('what is NOT about detachment', () => {
	test('the failing `insert` pair fails attached too', () => {
		...
		const twoInserts = (type: Y.Node) => {
			type.insert(0, ['hello ']);
			type.insert(6, ['world']);
		};
		expect(() => twoInserts(integrate(new Y.Node()))).toThrow(
			'Exceeded content range',
		);
		const detached = new Y.Node();
		twoInserts(detached);
		expect(() => integrate(detached)).toThrow('Exceeded content range');
	});
});

describe('what a fresh `new Y.Node()` gives you', () => {
	test('a fresh type, owned by no document', () => {
		expect(new Y.Node().doc).toBeNull();
		expect(new Y.Node()).not.toBe(new Y.Node());
	});
	...
	test('a whole conversion reaches a peer identically either way', () => {
		...
		const build = (detached: boolean): Y.Doc => {
			...
			document.transact(() =>
				root.setAttr('r' as never, new Y.Node() as never),
			);
			(root.getAttr('r' as never) as Y.Node).insert(0, children());
			return document;
		};
		const seenBy = (document: Y.Doc): string => {
			const peer = new Y.Doc({ gc: true });
			Y.applyUpdateV2(peer, Y.encodeStateAsUpdateV2(document));
			return (peer.get('root').getAttr('r' as never) as Y.Node).toString();
		};
