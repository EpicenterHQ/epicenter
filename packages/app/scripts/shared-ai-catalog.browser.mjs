...
const catalog = await createAiCatalog({
	...
	fetch: async (input, init) => {
		...
		return Response.json(
			{ data: [{ id: 'manual' }] },
			{ headers: { 'set-cookie': 'provider=must-not-escape' } },
		);
	},
});
...
const server = Bun.serve({
	hostname: '127.0.0.1',
	port: 0,
	fetch: (request) => route.fetch(request),
});
...
route = createHomeServer({
	...
	desktopAuth: auth,
	staticAssets: {
		homePage: '<!doctype html>Home',
		applications: ['whispering', 'vocab'].map((id) => ({
			id,
			title: id,
			page,
	...
	blobs() {
		throw new Error('No library blobs used by this acceptance.');
	},
	blobRemote: () => null,
	aiCatalog: catalog,
}).app;
...
try {
	assert.equal(
		(await fetch(`${origin}/_epicenter/ai/no-account/connections`)).status,
		401,
	);
	const boot = await fetch(BOOTSTRAP_ROUTE.url(origin), {
		method: 'POST',
		headers: { origin, authorization: 'Bearer test-launch-token' },
	});
	assert.equal(boot.status, 204);
	const cookie = boot.headers.get('set-cookie').split(';')[0];
	assert.equal(
		(
			await fetch(`${origin}/_epicenter/ai/no-account/connections`, {
				method: 'POST',
	...
	assert.equal(
		(
			await fetch(`${origin}/_epicenter/ai/no-account/connections`, {
				...
				body: JSON.stringify({ type: 'import', source: 'old', records: [] }),
			})
		).status,
		400,
	);
	...
	context.on('request', (request) => {
		if (request.url().endsWith('/_epicenter/ai/no-account/events'))
			eventRequests++;
	});
	...
	await context.addInitScript(() => {
		...
		localStorage.setItem(
			'whispering.app-ai-selections',
			JSON.stringify({
				version: 1,
				selections: { chat: { connectionId: 'legacy-one', model: 'manual' } },
			}),
		);
	...
	for (const tab of [whispering, vocab])
		tab.on('pageerror', (error) => errors.push(error.message));
	await Promise.all([
		whispering.goto(`${origin}/apps/whispering/`),
		vocab.goto(`${origin}/apps/vocab/`),
	]);
	for (const tab of [whispering, vocab]) {
		await tab.waitForFunction(() => window.acceptance);
		await tab.evaluate(() => window.acceptance.ready);
	}
	for (const tab of [whispering, vocab])
		assert.deepEqual(await tab.evaluate(() => window.acceptance.records()), []);
	const legacy = await whispering.evaluate(() => [
		localStorage.getItem('whispering.app-ai-connections'),
		localStorage.getItem('whispering.app-ai-selections'),
	]);
	assert.equal(
		await whispering.evaluate(() => window.acceptance.selected()),
		null,
	);
	...
	const removable = await vocab.evaluate(() =>
		window.acceptance.add({
			baseUrl: 'https://second.example/v1',
			models: ['manual'],
		}),
	);
	await whispering.evaluate(
		(id) => window.acceptance.select({ connectionId: id, model: 'manual' }),
		removable,
	);
	...
	await whispering.waitForFunction(
		(id) => window.acceptance.records().some((record) => record.id === id),
		id,
	);
	const record = await whispering.evaluate(
		(id) => window.acceptance.records().find((record) => record.id === id),
		id,
	);
	...
	assert.deepEqual(requests.at(-1), {
		url: 'https://shared.example/v1/models',
		key: 'Bearer shared-key',
		cookie: null,
	});
	await vocab.evaluate(
		(id) => window.acceptance.update(id, { name: 'Renamed' }),
		id,
	);
	await whispering.waitForFunction(
		(id) =>
			window.acceptance.records().find((record) => record.id === id)?.name ===
			'Renamed',
		id,
	);
	assert.equal(
		await whispering.evaluate(() => window.acceptance.runRetained()),
		'sent',
	);
	await vocab.evaluate(
		(id) => window.acceptance.update(id, { apiKey: 'rotated-key' }),
		id,
	);
	await whispering.waitForFunction(
		({ id, version }) =>
			window.acceptance.records().find((record) => record.id === id)
				?.accessVersion !== version,
		{ id, version: record.accessVersion },
	);
	assert.equal(
		await whispering.evaluate(() => window.acceptance.runRetained()),
		'retired',
	...
	await whispering.reload();
	await whispering.waitForFunction(() => window.acceptance);
	await whispering.evaluate(() => window.acceptance.ready);
	assert.equal(
		(await whispering.evaluate(() => window.acceptance.records())).some(
			(record) => record.id === 'legacy-one',
		),
		false,
	);
	assert.equal(
		(await whispering.evaluate(() => window.acceptance.records())).some(
			(record) => record.id === removable,
		),
		false,
	);
	assert.deepEqual(
		await whispering.evaluate(() => window.acceptance.selected()),
		{ connectionId: removable, model: 'manual' },
	);
	assert.deepEqual(await vocab.evaluate(() => window.acceptance.selected()), {
		connectionId: id,
		model: 'manual',
	});
	assert.deepEqual(
	...
	assert.equal(
		(await context.cookies()).some((cookie) => cookie.name === 'provider'),
		false,
	);
	const metadata = await readFile(
		join(evidence, 'ai/no-account/connections.json'),
		'utf8',
	);
	for (const key of ['legacy-key', 'shared-key', 'rotated-key', 'preview-key'])
		assert.equal(metadata.includes(key), false);
	await Promise.all([
		whispering.evaluate(() => window.acceptance.close()),
		vocab.evaluate(() => window.acceptance.close()),
	]);
	assert.deepEqual(errors, []);
	await catalog.close();
	const reopened = await createAiCatalog({ dataRoot: evidence, secrets });
	assert.equal(
		reopened.getAll().connections.some((record) => record.id === id),
		true,
	);
	assert.equal(
		reopened.getAll().connections.some((record) => record.id === 'legacy-one'),
		false,
	);
	assert.equal(
		reopened.getAll().connections.some((record) => record.id === removable),
		false,
	);
		...
		JSON.stringify(
			{
				passed: true,
				checks: [
					'host session and Origin enforcement',
					'obsolete import command rejected',
					'two test SPA documents',
					'idle SSE heartbeat',
					'initial catalog and live updates',
					'legacy values retained without adoption',
					'independent explicit selections',
					'key isolation',
					'rotation retirement',
					'preview',
					'reload without deleted connection resurrection',
					'catalog reopen',
				],
	...
	process.stdout.write(
		`Shared desktop AI browser acceptance passed: ${evidence}\n`,
	);
} finally {
	await browser?.close();
	await catalog.close();
