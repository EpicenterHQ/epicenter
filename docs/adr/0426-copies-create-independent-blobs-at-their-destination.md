...
- **Status:** Accepted
- **Date:** 2026-09-22
- **Amended by:** [ADR-0428](0428-whispering-recordings-reference-audio-in-their-containing-store.md) at the Whispering `remoteAudio` example; [ADR-0438](0438-hosted-blobs-have-stable-authority-urls.md) at hosted publication, where `publishPrivate` or `publishPublic` returns a fresh authority URL rather than `copyFrom` returning a store-relative BlobId. Copies still preserve bytes and leave the source in place.
