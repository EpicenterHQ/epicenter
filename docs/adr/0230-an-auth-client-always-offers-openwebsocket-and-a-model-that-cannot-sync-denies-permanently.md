...
- Callers must respect `permanence`. This is now the only guard, and it is not free: a caller that treats a permanent denial as an ordinary close will retry forever on backoff. The one caller is `packages/app/src/client-owned-data.ts`, which passes the account through as the transport; the classification itself lives in `packages/app/src/data/sync/attach.ts`, and treating a permanent denial as an ordinary close there would retry forever on backoff. Parking and resuming properly needs a signal in the `SyncDial` contract that `packages/data` does not yet expose.
