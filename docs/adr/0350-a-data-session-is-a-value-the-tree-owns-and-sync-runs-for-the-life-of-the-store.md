...
- **Status:** Accepted
- **Amended by:** [ADR-0353](0353-a-data-session-keeps-one-account-for-its-entire-lifetime.md) at the session key and transport lifetime: the tree keys on Account identity and passes that Account to `open(account)`. Retirement is permanent for that Account; the session still owns sync and teardown.
- **Date:** 2026-09-05
