...
without App. The remaining `openApp` still takes that schema to acquire
recording, blobs, SQL, secrets, and inference. Those capabilities do not need
tables or KV declarations.
...
Stores take the definition. Blob, SQL, and secret constructors take their
namespace ID. Recording takes its LocalBlobs destination. Inference takes its
...
open only SQL or blobs; it does not fabricate an empty declaration or construct
an App merely to reach those capabilities. A product composition function owns
its startup rollback and asynchronous workflows without becoming a mandatory
SDK constructor.

## Consequences

Schema inspection, artifacts, tests, and live stores share one declaration.
