...
- **Status:** Superseded
- **Superseded by:** [ADR-0393](0393-rows-refer-to-blobs-without-owning-their-lifetime.md) and [ADR-0366](0366-a-recorder-captures-into-its-explicit-local-blob-destination.md), revised 2026-09-17: Stop saves a completed recording as an independent app-local blob, then the application workflow creates a row containing its BlobId reference; unfinished capture is disposable. The row-first, no-cancel, and crash-finishes-it promises below are historical. This does not revive ADR-0184's single global capture slot.
- **Date:** 2026-08-03
...
- **Historical amendment before supersession:** [ADR-0349](0349-local-blobs-belong-to-the-app-on-this-device.md) at blob identity: withdrawn are "No blob identity crosses the boundary, ever. There is no `BlobId`" and the `audio.src = vocab.notes.blobUrl(note.id)` playback line. The recorder returns a `BlobId` at `stop`, the row's cell stores it, and playback goes through `epicenter.blobs.url(id)`. The row-that-fills rule, the `start` and `stop` semantics, the refusal of a `cancel` verb, and "a crash finishes it" stand.
