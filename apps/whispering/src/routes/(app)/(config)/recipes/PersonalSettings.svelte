<script lang="ts">
	...
	function remove(recipe: Recipe) {
		confirmationDialog.open({
			...
			onConfirm: async () => {
				personal.tables.recipes.delete(recipe.id);
...
<main class="flex w-full flex-1 flex-col gap-2 px-4 py-4 sm:px-8 mx-auto">
	...
	<Card class="flex flex-col gap-4 p-6">
		<div class="flex items-center justify-between gap-2">
			...
			{:else}
				<Button variant="outline" onclick={openConnection}
					>Sign in to create recipes</Button
				>
			{/if}
		</div>
...
<Modal.Root bind:open={editorOpen}>
	<Modal.Content>
		<Modal.Header>
			...
			<Modal.Description>
				A name and one instruction. The instruction is the whole recipe: text
				in, text out.
			</Modal.Description>
		</Modal.Header>
