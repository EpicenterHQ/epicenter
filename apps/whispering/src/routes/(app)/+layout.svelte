<script lang="ts">
	...
	async function selectLibrary(next: typeof library) {
		...
		if (!connecting) {
			const confirmed = await new Promise<boolean>((resolve) => {
				confirmationDialog.open({
					...
					description:
						'Changing libraries will close this page. Active recordings and unsaved work will be discarded.',
		...
		departingHref =
			location.pathname +
			(!auth.getState().account && next !== 'local' ? '?connect' : '');
...
{:else}
	...
	>
		...
		{#snippet children(app, account)}
			...
			{@const data = library === 'local' ? app.local : app.personal}
			{#if data}
