...
export function promotionFor(
	...
	return store.tables.capturePromotions.rows.find(
		(row) =>
...
export async function addToCapture(
	...
	if (!account || app.signal.aborted)
		throw new Error('Sign in to add text to Capture.');
	...
	if (store.persistence.get() !== 'saved')
		throw new Error(
			'The request is not saved locally yet. Finish saving before retrying.',
		);
	...
	try {
		...
		const acknowledged = new Promise<string>((resolve, reject) => {
			...
			timeout = setTimeout(
				() =>
					reject(
						new Error(
							'Capture did not confirm this request. Retry sends the same saved request.',
						),
					),
				20_000,
			);
			channel.onmessage = (event: MessageEvent<CapturePromotionMessage>) => {
				...
				if (
					...
					message.account.principalId !== request.account.principalId
				)
					return;
				if (message.type === 'inspection-required')
					reject(new PromotionInspectionRequired());
				else if (message.type === 'unavailable')
					reject(new PromotedCaptureUnavailable());
		...
		app.signal.throwIfAborted();
		const written = store.tables.capturePromotions.update(receipt.id, {
			captureId,
		});
...
export async function openInCapture(
	...
	const account = app.authAccount;
	if (!account || app.signal.aborted)
		throw new Error('Sign in to open Capture.');
	const channel = new BroadcastChannel(CAPTURE_PROMOTION_CHANNEL);
	const request = {
		type: 'open',
		requestId: crypto.randomUUID(),
	...
	try {
		...
		await new Promise<void>((resolve, reject) => {
			...
			app.signal.addEventListener('abort', onAbort, { once: true });
			timeout = setTimeout(
				() => reject(new Error('Capture did not open this entry. Try again.')),
				20_000,
			);
			channel.onmessage = (event: MessageEvent<CapturePromotionMessage>) => {
				const message = event.data;
				if (
					...
					message.account.principalId !== account.principalId
				)
					return;
				if (message.type === 'unavailable')
					reject(new PromotedCaptureUnavailable());
				else if (message.type === 'opened' && message.captureId === captureId)
					resolve();
			};
			channel.postMessage(request);
			interval = setInterval(() => channel.postMessage(request), 750);
...
export async function inspectCapture(app: WhisperingApp): Promise<void> {
	if (!app.authAccount || app.signal.aborted)
		throw new Error('Sign in to inspect Capture.');
	await setCaptureWindowVisible(true);
	app.signal.throwIfAborted();
}
